...
void PImax7219Control::testDisplay()
{
	...
	for (int address = 0; address < iDevices; address++)
	{
		...
		for (int j = 0; j < 8; j++)
		{ //turns on all digits and decimal points	
			_deviceHandle.setChar(address, j, ' ', true);
		}
	}
}
...
/* function for displaying a number on an 7 segment display
  INPUTS
  e7SegmentIDs DeviceNum - the number of the device to write to.  See enum e7SegmentIDs declarations
  int fNumber - the number to display.  If the number required is really a decimal, the value should be divided by 10^fPower accordingly.  Most notable in Mach speed.
  int fPower (optional) - the power of 10 with which to divide fNumber by to get decimal.

  this should be agnostic of the device if its truly an external function
  currently too much knowledge of the controlling s/w is necessary

  almost needs to be, devicehandle, DEVICEID, number, Power, negative, startindex, length
...
void PImax7219Control::displayNumber(int fDeviceID, long fNumber, int fStartIndex, int fLength, int fPower, int fNegative, bool fDEBUG)
{
	int lclDigits[8]; //length of a max7219 device, each digit is stored in a single entry of the array	
	//Serial.print("displayNumber...");
	Serial.println(fDeviceID);
