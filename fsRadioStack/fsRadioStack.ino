...
//  allRadios[COMM1].resolution = 250;
...
//  allRadios[COMM2].resolution = 250;
...
//  allRadios[XPDR].resolution = 1; //not used.  Always assumes +/-1 for each digit
...
void processSwitchPositions()
{
  ...
  Serial.print("Stby Frequency : ");
  Serial.println(allRadios[activeRadio].stbyFrequency);
    ...
    default:
      {
          ...
          // values are fractional and hence can be compared to >=1000 or <0
        {
          if (bIncrement)
          {
            if ((tempMinor + allRadios[activeRadio].resolution) >= 1000) tempMinor = 0;
            else tempMinor += allRadios[activeRadio].resolution;
          }
          else
          {
            if ((tempMinor - allRadios[activeRadio].resolution) < 0) tempMinor = (1000 - allRadios[activeRadio].resolution);
            else tempMinor -= allRadios[activeRadio].resolution;
          }
        }
        break;
